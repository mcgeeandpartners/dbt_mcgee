...
select 'NUMBER_OF_CUSTOMERS' as fact_name, region, fact_value
from number_of_customers

union all

select 'AVERAGE_LENGTH_CUSTOMER_RELATIONSHIP' as fact_name, region, fact_value
from average_length_customer_relationship

union all

select 'AVERAGE_DEAL_SIZE_ALL_TIME' as fact_name, region, fact_value
from average_deal_size_all_time

union all

select 'AVERAGE_ARR' as fact_name, region, fact_value * 12
from average_deal_size_all_time

union all

select 'AVERAGE_CUSTOMER_LIFETIME_VALUE' as fact_name, region, fact_value
from
    (
        select alcr.region, alcr.fact_value * adsa.fact_value as fact_value
        from average_length_customer_relationship alcr
        full join average_deal_size_all_time adsa on alcr.region = adsa.region
    ) average_customer_lifetime_value

union all

select 'Marketing Costs' as fact_name, null as region, sum(net_amount) as fact_value
from {{ ref("xero_overheads_dept") }}
where account_category = 'Marketing Costs'

union all

select 'AVERGAE_COST_OF_MARKETING_PER_CUSTOMER' as fact_name, null as region, fact_value
from
    (
        with
            marketing_costs as (
                select sum(net_amount) as fact_value
                from {{ ref("xero_overheads_dept") }}
                where account_category = 'Marketing Costs'
            )
...
select 'NRR' as fact_name, region, fact_value
from
    (
        select
            region,
            round(
                sum(modified_fy_2024_revenue) / nullif(sum(fy_2023_revenue), 0), 2
            ) as fact_value
