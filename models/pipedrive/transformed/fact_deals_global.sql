with
    ...
    number_of_customers as (
        ...
        from
            (
                select customer_name, count(*) as paid_months from base group by 1
            ) paid_companies
        where paid_months >= 6
    ),
    ...
    new_customers_2024 as (
        ...
        select count(distinct customer_name) as fact_value
        from nc_2024base
        where fiscal_year = 'FY2024' and ranking = 1
    ),
    marketing_costs as (
        select 'Marketing Costs' as fact_name, sum(net_amount) as fact_value
        from {{ ref("xero_overheads_dept") }}
        where
            account_category = 'Marketing Costs'
            -- exclude marketing resource costs
            and account_name != 'Marketing Resource Costs'
            and month >= '2023-07-01'
            and month <= '2024-06-01'
        union
        select
            'Marketing Costs' as fact_name, sum(total_marketing_expense) as fact_value
        from {{ ref("marketing_resource_cost") }}
    )
select 'AVERAGE_LENGTH_CUSTOMER_RELATIONSHIP' as fact_name, fact_value
from average_length_customer_relationship

union all

select 'AVERAGE_DEAL_SIZE_ALL_TIME' as fact_name, fact_value
from average_deal_size_all_time

union all

select 'AVERAGE_ARR' as fact_name, fact_value * 12
from average_deal_size_all_time

union all

select 'AVERAGE_CUSTOMER_LIFETIME_VALUE' as fact_name, fact_value
from
    (
...
select 'NUMBER_OF_CUSTOMERS' as fact_name, fact_value
from new_customers_2024

union all

(select fact_name, sum(fact_value) fact_value from marketing_costs group by 1)

union all

select 'AVERAGE_COST_OF_MARKETING_PER_CUSTOMER' as fact_name, fact_value
from
    (
        with
            marketing_costs_new as (
                select sum(fact_value) fact_value from marketing_costs
            )
...
select 'NRR' as fact_name, fact_value
from
    (
        select
            round(sum(modified_fy_2024_revenue) / sum(fy_2023_revenue), 2) as fact_value
