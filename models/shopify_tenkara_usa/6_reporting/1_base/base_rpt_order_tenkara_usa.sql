...
select
  ...
  {{ insert_shopify_last_click_attr_metric() }}
...
from {{ ref('fact_order_line_item_tenkara_usa') }} as oli
left join {{ ref('dim_date_tenkara_usa') }} as dates
	on oli.date_key = dates.date_key
left join {{ ref('base_rpt_customers_tenkara_usa') }} as customers
  on oli.customer_key = customers.customer_key
left join {{ ref('stg_order_url_tag_tenkara_usa') }} as mkt_attri
