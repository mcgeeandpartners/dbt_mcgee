...
from {{ref('stg_order_ad')}} as o 
left join {{ref('transform_order_line_item_ad')}} as oli 
    on o.order_id = oli.order_id
left join {{ref('dim_product_ad')}} as p 
    on oli.product_id = p.product_id
    and oli.product_variant_id = p.product_variant_id
left join {{ref('dim_customer_ad')}} as c 
    on o.customer_id = c.customer_id
left join {{ref('dim_date_ad')}} as d 
    on d."DATE" = o.order_placed_at_utc::date
{# left join {{ref('dim_discount_ad')}} as discount 
    on discount.order_id = o.order_id #}
 --The join with int_historic_msrp table is not needed for American Duchess since we are not using that here.
{# left join {{ ref('int_historic_msrp_ad') }} as i 
