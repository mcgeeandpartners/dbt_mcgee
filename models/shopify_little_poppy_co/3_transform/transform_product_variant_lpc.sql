...
with missing_variant_ids_int_order_line as (
    ...
    from {{ ref('int_order_line_item_lpc') }} as a 
    left join {{ ref('int_product_variant_lpc') }} as b 
...
, edge_cases as (
   select 
    ...
    from {{ ref('base_transform_null_variant_backfill_lpc') }}
)

select * from {{ ref('int_product_variant_lpc') }}
union all 
