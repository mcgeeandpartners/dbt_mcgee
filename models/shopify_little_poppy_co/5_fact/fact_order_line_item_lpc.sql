...
from {{ref('stg_order_lpc')}} as o 
left join {{ref('int_order_line_item_lpc')}} as oli 
    on o.order_id = oli.order_id
left join {{ref('dim_product_lpc')}} as p 
    on oli.product_id = p.product_id
    and oli.product_variant_id = p.product_variant_id
left join {{ref('dim_customer_lpc')}} as c 
    on o.customer_id = c.customer_id
left join {{ref('dim_date_lpc')}} as d 
