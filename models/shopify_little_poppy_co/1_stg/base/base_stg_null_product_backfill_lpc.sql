...
--For some product titles, we have null product ids in some orders, but populated product ids in others. With following 3 ctes, we are getting those product titles so we can add them back in final cte.

with source as (
    select * from {{ source('shopify_little_poppy_co', 'order_line') }}
),
